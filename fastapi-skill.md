...
Use **URL path versioning** (`/api/v1/...`, `/api/v2/...`). It's explicit, cache-friendly, and the most common pattern for REST APIs.
...
├── api/
│   ├── v1/
│   │   ├── __init__.py        # v1_router = APIRouter()
│   │   ├── auth/
│   │   │   ├── __init__.py    # auth_router aggregation
│   │   │   └── auth.py        # endpoints
│   │   └── users/
│   │       ├── __init__.py
│   │       └── users.py
│   └── v2/
│       ├── __init__.py        # v2_router = APIRouter()
│       └── users/
│           └── users.py       # breaking changes live here
├── main.py
```
...
# api/v2/__init__.py
from fastapi import APIRouter
from api.v2.users import users_router as v2_users_router

v2_router = APIRouter()
v2_router.include_router(v2_users_router, prefix="/users", tags=["Users v2"])
```
...
| Rule | Detail |
|------|--------|
| **New endpoint** | Always add to the **latest** version |
| **Non-breaking change** | Add fields, new endpoints → same version, no new route |
| **Breaking change** | Changed response shape, renamed fields, removed endpoint → **new version** |
| **Shared logic** | Put in `logic/` or `service/` layer, not in router — both v1 and v2 call the same logic |
| **Deprecation** | Old version stays functional; add `deprecated=True` to router |

### Deprecation

```python
# When v1 is deprecated but still served:
v1_router = APIRouter(deprecated=True)  # Shows strikethrough in /docs
```
...
Non-breaking (same version):        Breaking (new version):
  + Add new optional field            × Remove or rename field
  + Add new endpoint                  × Change field type
  + Add query parameter               × Change HTTP method
  + Widen enum values                 × Change URL path structure
```
...
| Breaking change in existing version | Clients relying on v1 break silently. | Create `/api/v2/` for breaking changes; mark v1 router `deprecated=True`. |
| Duplicating logic across versions | Bug fixes must be applied in N places. | Shared logic belongs in `logic/` or `service/` layer; both versions call the same function. |
...
| API versioning                       | URL path (`/api/v1/`, `/api/v2/`); new version only for breaking changes |
